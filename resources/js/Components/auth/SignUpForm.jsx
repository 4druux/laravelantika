...
export default function SignUpForm() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Blok ini menangani semua jenis error dari fungsi register

            // Atur state loading kembali ke false
            setIsLoading(false);

            // 1. Cek apakah ini error validasi (dari status 422) yang memiliki properti 'errors'
            if (error.errors) {
                // Perbarui state errors untuk menampilkan pesan di bawah input field yang sesuai
                setErrors(error.errors);

                // Ambil pesan error spesifik yang paling relevan untuk ditampilkan di atas form
                const specificErrorMessage =
                    error.errors.email?.[0] || "Validasi gagal.";
                setAuthError(specificErrorMessage);
                toast.error(specificErrorMessage);
            } else if (error.message) {
                // 2. Jika bukan error validasi, tampilkan pesan error generik dari API
                setAuthError(error.message);
                toast.error(error.message);
            } else {
                // 3. Pesan fallback jika tidak ada properti error yang dikenali
                setAuthError("Registrasi gagal. Coba lagi.");
